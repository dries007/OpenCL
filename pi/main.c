...
double estimate_pi(const cl_uint n)
{
    ...
    cl_float * inpx = calloc(n, sizeof(cl_float));
    cl_float * inpy = calloc(n, sizeof(cl_float));
    cl_uint * outp = calloc(groups, sizeof(cl_uint));

    srand((unsigned int)time(NULL));
    for (int i = 0; i < n; i++)
    {
        inpx[i] = ((float)rand()/(float)RAND_MAX);
        inpy[i] = ((float)rand()/(float)RAND_MAX);
    }

    cl_int error;

    // Input buffer on GPU
    cl_mem dev_inpx = clCreateBuffer(g_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float) * n, inpx, &error);
    ocl_err(error);
    cl_mem dev_inpy = clCreateBuffer(g_context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_float) * n, inpy, &error);
    ocl_err(error);
    ...
    for (int i = 0; i < groups; i++)
    {
        trues += outp[i];
        _n += group_size;
//        printf("%d (%d/%d) %lf\n", outp[i], trues, _n, 2.0 * ((double)trues / (double)_n));
    }
